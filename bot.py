...
BOT_TOKEN = os.environ.get('BOT_TOKEN')  # REQUIRED ADMINS_RAW = os.environ.get('ADMIN_IDS', '')  # comma-separated admin ids ACCOUNT_MOBILE = os.environ.get('ACCOUNT_MOBILE', '00799999004268889017')

Parse admin ids into a set of ints

ADMINS = set() for part in ADMINS_RAW.split(','): part = part.strip() if not part: continue try: ADMINS.add(int(part)) except ValueError: # ignore invalid values pass

Fallback: if ADMINS is empty, you can still add a default admin id here for testing.

if not ADMINS: # NOTE: remove this in production or set ADMIN_IDS env variable. try: ADMINS.add(7918198745) except Exception: pass

Paths to book PDFs (place your book files here)

BOOKS_DIR = Path(file).parent / 'books' BOOK_FILES = { 'marketing': BOOKS_DIR / 'marketing.pdf', 'product': BOOKS_DIR / 'product.pdf' }

Group links and responsible username

GROUP_LINKS = { 'marketing': 'https://t.me/+39YNXIC0CgJkNTdk', 'product': 'https://t.me/+c9rnGxHKsX5mYjA0' } RESPONSIBLE_USERNAME = '@aleeddin'  # contact for product buyers

DB_PATH = 'payments.db'

Prices

PRICES = { 'marketing': 1000, 'product': 1500 }

States (not using ConversationHandler but simple state flags in user_data)

CHOOSING = 'CHOOSING' WAITING_RECEIPT = 'WAITING_RECEIPT' WAITING_EMAIL = 'WAITING_EMAIL'
...
async def init_db(): async with aiosqlite.connect(DB_PATH) as db: await db.execute(''' CREATE TABLE IF NOT EXISTS payments ( id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, full_name TEXT, book TEXT, payment_time TEXT, status TEXT, receipt_file_id TEXT, email TEXT, verified_by INTEGER ) ''') await db.commit()

async def add_payment(user_id, username, full_name, book, payment_time, status, receipt_file_id): async with aiosqlite.connect(DB_PATH) as db: cur = await db.execute(''' INSERT INTO payments (user_id, username, full_name, book, payment_time, status, receipt_file_id) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (user_id, username, full_name, book, payment_time, status, receipt_file_id)) await db.commit() rowid = cur.lastrowid await cur.close() return rowid

async def update_payment_status(payment_id, status, verified_by=None): async with aiosqlite.connect(DB_PATH) as db: if verified_by is None: await db.execute('UPDATE payments SET status = ? WHERE id = ?', (status, payment_id)) else: await db.execute('UPDATE payments SET status = ?, verified_by = ? WHERE id = ?', (status, verified_by, payment_id)) await db.commit()

async def set_payment_email(payment_id, email): async with aiosqlite.connect(DB_PATH) as db: await db.execute('UPDATE payments SET email = ? WHERE id = ?', (email, payment_id)) await db.commit()

async def get_payment(payment_id): async with aiosqlite.connect(DB_PATH) as db: cur = await db.execute('SELECT * FROM payments WHERE id = ?', (payment_id,)) row = await cur.fetchone() await cur.close() return row

----------------- Helpers ------------------

def is_valid_email(email: str) -> bool: pattern = r"^[\w.-]+@[\w.-]+.\w{2,}$" return bool(re.match(pattern, email))

async def download_and_check_image(bot, file_id): """Download a Telegram file by file_id and try to determine if it's a valid image. Returns format string like 'JPEG'/'PNG' or None if not an image. """ try: file = await bot.get_file(file_id) data = await file.download_as_bytearray() img = Image.open(io.BytesIO(data)) return img.format except Exception: return None

------------------ Bot Handlers ------------------

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [ [KeyboardButton('معلومات'), KeyboardButton('شراء الكتاب')] ] reply = ReplyKeyboardMarkup(keyboard, resize_keyboard=True) await update.message.reply_text( 'أهلاً وسهلاً 👋\n' 'أنا مساعد مشروعنا لتأهيل الشباب في التسويق وصناعة المنتجات الرقمية.\n' 'اختر أحد الخيارات:', reply_markup=reply ) # clear any user-specific state context.user_data.clear() context.user_data['state'] = CHOOSING

async def info_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text( 'هذا المشروع يهدف إلى تدريب الشباب في مجالي التسويق وصنع المنتجات الرقمية.\n' f'⚠️ يجب أن تمتلك حساب Gumroad قبل المتابعة.\n' f'⚠️ الدفع يكون عن طريق التحويل إلى الحساب البريدي/موب: {ACCOUNT_MOBILE}\n' 'يفضل أن تمتلك اسم مستخدم في تلغرام ليسهل التواصل.\n\n' 'اضغط "شراء الكتاب" للمتابعة.' ) context.user_data['state'] = CHOOSING

async def buy_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [ [KeyboardButton('كتاب التسويق'), KeyboardButton('كتاب صنع المنتجات')], [KeyboardButton('العودة للرئيسية')] ] reply = ReplyKeyboardMarkup(keyboard, resize_keyboard=True) await update.message.reply_text('اختر الكتاب الذي تريد شراءه:', reply_markup=reply) context.user_data['state'] = CHOOSING

async def choose_book(update: Update, context: ContextTypes.DEFAULT_TYPE): text = update.message.text.strip() if text == 'العودة للرئيسية': return await start(update, context)
...
async def received_photo_or_doc(update: Update, context: ContextTypes.DEFAULT_TYPE): # Accept both photos and documents (pdf) user = update.message.from_user state = context.user_data.get('state') if state != WAITING_RECEIPT: await update.message.reply_text('لست في مرحلة دفع الآن. ابدأ بالضغط على "شراء الكتاب" إذا رغبت.') return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query user = query.from_user data = query.data await query.answer()
...
if action == 'accept':
    ...
    # Find where the user_data for that buyer is stored. Since admin and user have separate contexts,
    # we cannot directly access the buyer's user_data here. We'll send a message to the buyer telling them they were rejected,
    # and when they re-submit we will decrement attempts stored in DB? To keep simple, we'll use a per-user in-memory attempts map.
    ...
    if attempts > 0:
        ...
        try:
            ...
            # change user state so next file is accepted as receipt
        except Exception:
            pass
    else:
        ...
        try:
            await context.bot.send_message(chat_id=buyer_id, text='انتهت محاولاتك. تم إرجاعك إلى الصفحة الرئيسية.')
            # optional: show start keyboard
            await start(update=Update(update.update_id, None), context=context)
        except Exception:
            pass

Text handler for various commands and for receiving the Gumroad email after acceptance

async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE): text = (update.message.text or '').strip()
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE): logger.error('Exception while handling an update:', exc_info=context.error) try: if isinstance(update, Update) and getattr(update, 'message', None): await update.message.reply_text('حدث خطأ غير متوقع. يرجى المحاولة لاحقاً.') except Exception: pass

------------------ Main ------------------

async def main_async(): if not BOT_TOKEN: logger.error('BOT_TOKEN environment variable not set. Aborting.') return
